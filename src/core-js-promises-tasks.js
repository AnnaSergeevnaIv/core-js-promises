...
function getFirstResolvedPromiseResult(promises) {
  return Promise.any(promises).then((promise) => promise);
}
...
function getFirstPromiseResult(promises) {
  return Promise.race(promises).then((promise) => promise);
}
...
function queuePromises(promises) {
  return promises.reduce(
    (sum, curr) =>
      sum.then((value) =>
        curr.then((value1) => Promise.resolve(`${value}${value1}`))
      ),
    Promise.resolve('')
  );
}
